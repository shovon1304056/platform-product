...
const superAdminModel = require("./super-admin");
const commonObject = require('../common/common');

let databaseColum = [{ "name": "id", "type": "int" }, { "name": "role_id", "type": "int" },
{ "name": "profile_id", "type": "int" }, { "name": "email", "type": "varchar" },
{ "name": "password", "type": "text" },
{ "name": "status", "type": "int" },
{ "name": "updated_by", "type": "int" }, { "name": "updated_at", "type": "datetime" }];
let jsonColum = [];


let getUserByUserNameOrEmail = async (userName = "") => {
    return new Promise((resolve, reject) => {
        connectionProductOperationMYSQL.query(queries.getUserByUserNameOrEmail(), [userName, userName], (error, result, fields) => {
...
let getUserById = async (id = 0) => {
    return new Promise((resolve, reject) => {
        connectionProductOperationMYSQL.query(queries.getUserById(), [id], (error, result, fields) => {
            if (error) reject(error)
            else resolve(result)
        });
    });
...
let getUserDataById = async (id = 0) => {
    return new Promise((resolve, reject) => {
        connectionProductOperationMYSQL.query(queries.getUserDataById(), [id], (error, result, fields) => {
            if (error) reject(error)
            else resolve(result)
        });
    });
...
let getUserDetailsById = async (id = 0) => {
    return new Promise((resolve, reject) => {
        connectionProductOperationMYSQL.query(queries.getUserDetailsById(), [id], (error, result, fields) => {
            if (error) reject(error)
            else resolve(result)
        });
    });
...
let getUserByEmail = async (email = "") => {
    return new Promise((resolve, reject) => {
        connectionProductOperationMYSQL.query(queries.getUserByEmail(), [email], (error, result, fields) => {
            if (error) reject(error)
            else resolve(result)
        });
    });
