...
const queries = require('../queries/product-category');


let databaseColum = [{"name":"id","type":"int"},{"name":"title","type":"varchar"},{"name":"image","type":"varchar"},{"name":"status","type":"int"},{"name":"created_by","type":"int"},{"name":"updated_by","type":"int"},{"name":"created_at","type":"datetime"},{"name":"updated_at","type":"datetime"}];
let jsonColum = [];
...
let getActiveList = async () => {
    return new Promise((resolve, reject) => {
        connectionProductOperationMYSQL.query(queries.getActiveList(), (error, result, fields) => {
            if (error) reject(error)
            else resolve(result)
        });
    });
...
let getById = async (id = 0) => {
    return new Promise((resolve, reject) => {
        connectionProductOperationMYSQL.query(queries.getById(), [id], (error, result, fields) => {
            if (error) reject(error)
            else resolve(result)
        });
    });
