...
const queries = require('../queries/product-image');


let databaseColum = [{ "name": "id", "type": "bigint" }, { "name": "product_id", "type": "bigint" }, { "name": "public_id", "type": "varchar" }, { "name": "url", "type": "text" }, { "name": "status", "type": "int" }];
let jsonColum = [];
// Promises Method
...
let addNew = async (info) => {
    return new Promise((resolve, reject) => {
        connectionProductOperationMYSQL.query(queries.addNew(), [info], (error, result, fields) => {
            if (error) reject(error)
            else resolve(result)
        });
    });
