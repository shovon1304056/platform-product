...
const productHighlightModel = require("./product-highlight");
const productImageModel = require("./product-image");


let databaseColum = [{ "name": "id", "type": "bigint" }, { "name": "category_id", "type": "bigint" }, { "name": "brand_id", "type": "bigint" }, { "name": "name", "type": "varchar" }, { "name": "description", "type": "text" }, { "name": "price", "type": "decimal" }, { "name": "cutted_price", "type": "decimal" }, { "name": "stock", "type": "int" }, { "name": "warranty", "type": "int" }, { "name": "ratings", "type": "decimal" }, { "name": "num_of_reviews", "type": "int" }, { "name": "created_by", "type": "bigint" }, { "name": "updated_by", "type": "bigint" }, { "name": "created_at", "type": "datetime" }, { "name": "updated_at", "type": "datetime" }, { "name": "status", "type": "int" }];
let jsonColum = [];
...
let getList = async () => {
    return new Promise((resolve, reject) => {
        connectionProductOperationMYSQL.query(queries.getList(), (error, result, fields) => {
...
let addNew = async (info, conn = undefined) => {
    let connection = connectionProductOperationMYSQL;
...
let getDetailsByIdAndWhereIn = async (expertTypeId = []) => {
    return new Promise((resolve, reject) => {
        connectionProductOperationMYSQL.query(queries.getDetailsByIdAndWhereIn(), [expertTypeId], (error, result, fields) => {
            if (error) reject(error)
            else resolve(result)
        });
    });
...
let addWithMultipleInfo = async (data = {}, highlightsArray = [], imageArray = []) => {
    return new Promise((resolve, reject) => {

        connectionProductOperationMYSQL.getConnection(function (err, conn) {
            ...
            conn.beginTransaction(async function (error) {
                ...
                let result = await addNew(data, conn);
                ...
                if (!isEmpty(highlightsArray)) {
                    for (let index = 0; index < highlightsArray.length; index++) {
                        const element = highlightsArray[index];

                        element.product_id = productId;

                        let highlightsInformation = await productHighlightModel.addNew(element, conn);

                        if (highlightsInformation.affectedRows == undefined || highlightsInformation.affectedRows < 1) {
                            return conn.rollback(function () {
                                conn.release();
                                resolve(highlightsInformation);
                            });
                        }
                    }
                }
                ...
                if (!isEmpty(imageArray)) {
                    for (let index = 0; index < imageArray.length; index++) {
                        ...
                        let imageInformation = await productImageModel.addNew(element, conn);

                        if (imageInformation.affectedRows == undefined || imageInformation.affectedRows < 1) {
                            return conn.rollback(function () {
                                conn.release();
                                resolve(imageInformation);
                            });
                        }
                    }
                }
