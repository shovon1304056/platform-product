...
let table_name = "staffingdb_users";


let getUserByUserName = () => {
    return `SELECT id, role_id, profile_id, email,status  FROM ${table_name} where  user_name = ?`;
}

let getUserByUserNameOrEmail = () => {
    return `SELECT id, role_id, profile_id, email,status FROM ${table_name} where  user_name = ? or email = ?`;
}

let getUserById = () => {
    return `SELECT id, role_id, profile_id, email, password ,status FROM ${table_name} where  id = ?  and status = 1 `;
}


let getUserDetailsById = () => {
    return `SELECT id, role_id, profile_id, email ,status FROM ${table_name} where  id = ?   `;
}

let getPendingUserById = () => {
    return `SELECT id, role_id, profile_id, email ,status FROM ${table_name} where  id = ?  and status = 2 `;
}
...
let getDataByWhereCondition = (data = {}, orderBy = {}, limit, offset, columnList = []) => {
    ...
    if (!["skip", "SKIP", "Skip"].includes(limit)) query += `LIMIT ${offset}, ${limit}`;
